...
fprintf('Select .mat file output from "HD1 Output files" (e.g. 04KS_KDTam_KDT.mat)');
...
prompt = {'Enter Subject ID:','Enter Condition ID: i.e. KDTam MCI'};
dlgtitle = 'Input conditions';
dims = [1 35];
definput = {'02JK','KDTam MCI'};
...
if channels_remove == 1 
    ...
    fprintf('Select .xlsx file with the names of channels to remove (e.g. EEG100) \n Ensure that channel names are taken from the reference list of channel names and numbers.'); 
...
%% plot spectrum for eyes open, eyes close, overall
%'1 eyes open; 0 eyes closed; 3 overall;

stages = [1,0,3];
stage_names ={'eyes_open','eyes_closed','overall'};

% add if statement in case the data is empty 

outchans =[];
len_chans =[];
for i = 1: length(stages)
    if stages(i) == 3
        idx_stage = [find(sleep_scores==1) find(sleep_scores==0)];
    else
    ...
    if sum(~isnan(data_selection(:))) == 0
        sprintf('No data in %s', char(stage_names(i)));
    elseif sum(~isnan(data_selection(:))) > 0
        ...
        %plot_bands_spectra_ind_headModel_TK(EEG)
...
output_stages = {'eyes_open','eyes_closed','overall'};
...
elseif channels_plot == 2 % defined good 164 channels
    save_outchansname = fullfile(figpath,...
    [subjectID,'_',EEG.condition,'_outlier_goodchans.txt']);
...
saveID = sprintf('%s-Condition_%s-%s',subjectID,conditionID,channels_label);
